...
1. **5-point band thresholds** — deck fixes Hold=[-0.75,+0.75] & extremes ±2.0 but not the Strong/regular boundaries. Proposed default ±1.5 (see `schema.sql` footer). Needs A1/A2 confirm.
2. Human confirms (per A1): deck v1.0 client-final? default horizon 6mo / weights 30/30/25/15? Azure Japan East + $80/mo budget?
